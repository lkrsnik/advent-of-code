import copy, time, heapq, sys, time
...
class TreeNode:
    ...
    def countDistanceWidthSearch(self, goal):
        visited_nodes = []
        extreme_nodes = [self]
        for depth in range(500):
            # print str(depth)+': '+ str(len(extreme_nodes)) + ' - ' + str(len(visited_nodes))
            new_extreme_nodes = []
            for node in extreme_nodes:
                # time.now
                if node != None:
                    # print 'Node: ' + str(node.x) + ':' + str(node.y)
                    ...
                    if node.rightNode is not None:
                        ...
                        new_extreme_node, goal_node = node.rightNode.testNode(visited_nodes, new_extreme_nodes, goal)
                        # print("--- testNode = %s seconds ---" % (time.time() - start_time))
                        if goal_node:
                            return depth
                        if new_extreme_node:
                            new_extreme_nodes.append(new_extreme_node)

                    if node.upNode is not None:
                        new_extreme_node, goal_node = node.upNode.testNode(visited_nodes, new_extreme_nodes, goal)
                        if goal_node:
                            return depth
                        if new_extreme_node:
                            new_extreme_nodes.append(new_extreme_node)
                    ...
                    if node.leftNode is not None:
                        new_extreme_node, goal_node = node.leftNode.testNode(visited_nodes, new_extreme_nodes, goal)
                        if goal_node:
                            return depth
                        if new_extreme_node:
                            new_extreme_nodes.append(new_extreme_node)
                    if node.downNode is not None:
                        new_extreme_node, goal_node = node.downNode.testNode(visited_nodes, new_extreme_nodes, goal)
                        if goal_node:
                            return depth
                        if new_extreme_node:
                            new_extreme_nodes.append(new_extreme_node)
                    visited_nodes.append(node)
            extreme_nodes = new_extreme_nodes


    def testNode(self, visited_nodes, new_extreme_nodes, goal):
        if self in visited_nodes or self in new_extreme_nodes:
            # if self in new_extreme_nodes and self not in visited_nodes:
            #     print 'HERE!!!'
            return False, False
        # extreme_nodes.append(self)
    ...
    def getDistance(self, i, j):
        ...
        return start_node.countDistanceWidthSearch(j)
...
def solve1():
    ...
    with open("input2", "r") as filestream:
        ...
        #             dist_line.append(0)
        #         else:
        #             dist_line.append(tree_structure.getDistance(i, j))
        #             # distances[i][j] =
        #             # print str(i) + ':' + str(j)
        #             # print distances[i][j]
    ...
    distances = [[0, 173, 57, 215, 151, 53, 215, 239], [0, 0, 215, 69, 53, 211, 69, 93], [0, 0, 0, 257, 193, 19, 257, 281], [0, 0, 0, 0, 79, 253, 23, 47], [0, 0, 0, 0, 0, 189, 79, 103], [0, 0, 0, 0, 0, 0, 253, 277], [0, 0, 0, 0, 0, 0, 0, 31], [0, 0, 0, 0, 0, 0, 0, 0]]
    ...
    getWayCost([0], 0, distances, 0)
    minSum = 999999
    minSums = [999999] * len(distances)
    minPath = []
    for el in allPaths:
        if el['sum'] < minSum:
            minSum = el['sum']
            minPath = el['path']
        if el['sum'] + distances[0][el['path'][-1]] + len(distances[0]) < minSums[el['path'][-1]]:
            minSums[el['path'][-1]] = el['sum'] + distances[0][el['path'][-1]] + len(distances[0])
            # minSums[el['path'][-1]]

    print (minSum + len(distances[0]) - 1)
    print minPath
    print minSums
...
def getWayCost(path, row, distances, sum):
    # bestSum = 999999
    for col in range(len(distances[0])):
        path_instance = copy.deepcopy(path)
        if len(path) == len(distances[0]):
            allPaths.append({'sum': sum, 'path': path})
        if col not in path:
            path_instance.append(col)
            getWayCost(path_instance, col, distances, distances[row][col] + sum)
